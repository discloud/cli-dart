...
class MultiSelect extends Component<List<int>> {
  ...
  MultiSelect({required this.prompt, required this.options, this.defaults})
...
class _MultiSelectState extends State<MultiSelect> {
  ...
  void init() {
    ...
    if (component.defaults case final defaults?) {
      if (defaults.length != component.options.length) {
        throw Exception(
          "Default selections have a different length of ${defaults.length} than options of ${component.options.length}",
        );
      } else {
        selection.addAll(defaults.indexed.where((e) => e.$2).map((e) => e.$1));
      }
    }

    context
      ..writeln(promptInput(theme: component.theme, message: component.prompt))
      ..hideCursor();
  }
