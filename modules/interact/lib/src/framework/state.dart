...
abstract class State<T extends Component> {
  ...
  Context get context {
    if (_context case final context?) return context;

    throw Exception(
      "The state's context is already disposed or is not created initially.",
    );
  }
  ...
  T get component {
    if (_component case final component?) return component;

    throw Exception(
      "The state is not bind to a component or is already disposed.",
    );
  }
  ...
  @protected
  @mustCallSuper
  void setState([void Function() fn = _noop]) {
    fn();
    context.wipe();
    render();
