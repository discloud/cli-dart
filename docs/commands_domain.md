# [CLI Documentation](index.md)

## [Commands](commands.md)

### [domain](commands_domain.md)

- [create](commands_domain_create.md)
- [delete](commands_domain_delete.md)
- [edit](commands_domain_edit.md)
- [info](commands_domain_info.md)
- [verify](commands_domain_verify.md)

```sh
Manage your domains

Usage: discloud domain <subcommand> [arguments]
-h, --help    Print this usage information.

Available subcommands:
  create   Create a domain
  delete   Delete a domain
  edit     Edit a domain
  info     Get information of your domains
  verify   Verify a domain

Run "discloud help" to see global options.
```
